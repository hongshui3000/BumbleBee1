...
uint8_t      ancs_notificationEnable = 0;


bool ancs_enableNotification(uint16_t cccHandle)
{
    void       * pBuffer;        
    uint16_t cccValue = 0x0001;
...
#endif
                    ancs_notificationEnable = 0;
                    ancs_enableNotification(ancs_ds_value_handle+1);
                }
                break;
...
#endif
    if(!ancs_notificationEnable)
    {
        ancs_enableNotification(ancs_ns_value_handle+1);
        ancs_notificationEnable = true;
    }
    return( true );
}
