...
extern void hmt_set_mtu_size(uint16_t size);
#endif
...
#if PROFILE_SUPPORT_HOMEKIT
/**< homekit pairing service id */
uint8_t HomekitPairingServiceID = 0;
/* Function pointer used to send handle prepare write & execute write in homekit pairing. */
gattPreExeWrCB_t pfnPsPreExeWrCB = {NULL, NULL};
#endif
...
#if PROFILE_SUPPORT_HOMEKIT
    if(HomekitPairingServiceID == serviceIdx)
    {
        if(pfnPsPreExeWrCB.pfnPreWriteAttrCB)
        {
            appResult = (TAppResult)pfnPsPreExeWrCB.pfnPreWriteAttrCB(serviceIdx, handle, iAttribIndex, wLength, pValue, offset);
        }
        else
        {
            DBG_BUFFER(MODULE_PROFILE, LEVEL_ERROR, "-->pfnPsPreExeWrCB.pfnPreWriteAttrCB = NULL\n", 0);
        }
    }
    else
    {
...
#if PROFILE_SUPPORT_HOMEKIT
    if(pfnPsPreExeWrCB.pfnExeWriteAttrCB)
    {
        appResult = (TAppResult)pfnPsPreExeWrCB.pfnExeWriteAttrCB(flag, handle);
    }
    else
    {
        DBG_BUFFER(MODULE_PROFILE, LEVEL_ERROR, "-->pfnPsPreExeWrCB.pfnPreWriteAttrCB = NULL\n", 0);
...
#if PROFILE_SUPPORT_HOMEKIT
    hmt_set_mtu_size(pConnectMDLInfo->maxTPDUSize);
#endif
