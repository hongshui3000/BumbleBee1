...
#define MAX_CONN_INTERVAL             0x0C80  // minimum connection interval
#define DEFAULT_TIMEOUT_MULTIPLIER    1000    // default timeout multiplier
...
/**
 * @brief   Get a parameter in the Central Profile Role.
 *
 * @param   param - profile parameter ID: @ref GAPCENTRALROLE_PROFILE_PARAMETERS
 * @param   len - length of the parameter to get
 ...
 * @internal
 *
 * @brief   the registered callback will be used to send message to application directly from central role.
 *
 * @param   pAppCallback - Callback app provided by application
 *
 */
 ...
 *
 * @return  gapAPI_CauseSuccess: Connection update started.<BR>
 *          gapAPI_IncorrectMode: No connection to update.<BR>
 */
 ...
 *
 * @return  TRUE: send request success.<BR>
 *          FALSE: send request failed.<BR>
 */
 ...
 *
 * @return  gapAPI_CauseSuccess: Terminate started.<BR>
 *          gapAPI_IncorrectMode: No link to terminate.<BR>
 */
...
TGAP_STATUS central_SetTxPower( uint8_t tx_power_index )
{
    TGAP_STATUS ret = gapAPI_CauseSuccess;
...
static void central_Handle_LEScanInfo(PBlueAPI_LEScanInfo pLeScanInfo)
{
    ...
    {
        msgType = CENTRAL_ADV_SCAN_RSP_DATA_MSGTYPE;
...
static void central_Handle_LEConnectionUpdateInd(PBlueAPI_LEConnectionUpdateInd pConnectionUpdateInd)
{
    ...
    if(pGapCentralAppDirectCb)
    {
        msgType = CENTRAL_CONN_PARAM_CONFIRM_MSGTYPE;
