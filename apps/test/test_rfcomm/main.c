...
#include "test_rfcomm_application.h"
#include "dip.h"
...
void GAP_Init(void)
{
    uint8_t  device_name[GAP_DEVICE_NAME_LEN] = "rfcomm";
...
{
    ///TODO: add legacy profile init here, in which add SDP record is needed
	TestProfileInit();
	rfcomm_app_init();
...
* @return  void
*/
int TestRfcomm_main(void) //Testlegacy_main(void)
{
    Board_Init();
    Driver_Init();
    legacy_Init();
