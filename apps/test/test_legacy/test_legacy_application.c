...
TAppResult AppProfileCallback(uint8_t service_id, void *p_data)
{
    if (service_id == ProfileAPI_ServiceUndefined)
    {
        ...
        uint8_t *extra_data = (uint8_t *)osMemoryAllocate(RAM_TYPE_DATA_ON, 5);
        extra_data[0] = 0x03;
        extra_data[1] = 0x01;
        extra_data[2] = 0x02;
        extra_data[3] = 0x03;
        ...
        case PROFILE_EVT_SDP_REG_COMPLETE:
            ...
            legacy_SetDIDEIR(0x1111, 0x0001, 0x1122, 0x2233);
            legacy_SetExtraEIR(extra_data);
...
void TestProfileInit(void)
{
    ...
    length = legacy_SDPRecordLength("< I<U> I<<UI><UI>> I<U> I<III> I<<UI>> IS>",
                                  ...
                                  SDP_ATTR_SERVICENAME + SDP_BASE_LANG_OFFSET , "a2dp source" /* English Servicename */
                                  );
    DBG_BUFFER(MODULE_APP, LEVEL_INFO, "TestProfileInit: SDP buffer length cal is %d", 1, length);
    ...
    length = legacy_SDPRecordLength("< I<U> I<<UI><UI>> I<U> I<III> I<<UI>> IS>",
                                      ...
                                      SDP_LANGUAGE_ENGLISH, SDP_CHARCODE_UTF8, SDP_BASE_LANG_OFFSET,
                                      SDP_ATTR_BLUETOOTHPROFILEDESCRIPTORLIST, UUID_ADVANCEDAUDIODISTRIBUTION, 0x0100 /* version 1.0 */,
                                      SDP_ATTR_SERVICENAME + SDP_BASE_LANG_OFFSET , "a2dp sink" /* English Servicename */
                                      );
    DBG_BUFFER(MODULE_APP, LEVEL_INFO, "TestProfileInit: SDP buffer length cal is %d", 1, length);
