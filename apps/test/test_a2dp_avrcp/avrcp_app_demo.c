...
void VolumeUp(TAvrcpLink *p_link_to_notify)
{
    if (App.volume == 0x7f)
    {
    ...
    if (p_link_to_notify->vol_change_registered)
    {
        p_link_to_notify->vol_change_registered = false;
        avrcp_NotifyVolumeChange(p_link_to_notify->remote_bd, App.volume);
    }
}
void VolumeDown(TAvrcpLink *p_link_to_notify)
{
    if (App.volume == 0)
    {
        return; //do not send EVENT_VOLUME_CHANGED
...
bool CmdAction_SendVendorCmdKeyAct(TAvrcpLink *p_link, uint8_t para)
{
    uint8_t pdu[2] = {
        VENDOR_CMD_KEY_ACT,
        para
    };
    return avrcp_SendVendorCommand(p_link->remote_bd, AVRCP_CONTROL, MY_COMPANY_ID, pdu, sizeof(pdu)) == AVRCP_SUCCESS ? true : false;
}
...
void KeyPress(TAvrcpLink *p_link, uint8_t op_id) //auto start/stop resend timer
{
    App.p_key_hold_resend_link = p_link;
    App.key_hold_resend_op_id = op_id;
    xTimerStart(App.key_hold_resend_timer, 0);
    PendCmdAction(p_link, CmdAction_Passthrough, (PRESSED << 7) | op_id);
}
void KeyRelease(TAvrcpLink *p_link, uint8_t op_id)
{
    xTimerStop(App.key_hold_resend_timer, 0);
    App.key_hold_resend_op_id = 0;
    App.p_key_hold_resend_link = NULL;
    PendCmdAction(p_link, CmdAction_Passthrough, (RELEASED << 7) | op_id);
}

void KeyHoldResendTimeout(xTimerHandle timer) //not auto reload: timeout->resend key press->start timer again
{
    KeyPress(App.p_key_hold_resend_link, App.key_hold_resend_op_id);
...
void HandleRspUnitInfo(TAvrcpLink *p_link, const TRspUnitInfo *rsp) //app specific
{
    if (rsp->state == AVRCP_RSP_STATE_SUCCESS)
    {
        ...
        if (rsp->company_id != MY_COMPANY_ID) //remote is phone
        {
            App.p_phone_link = p_link;
        ...
        } else //remote is partner
        {
            App.p_partner_link = p_link;
            printf("Partner found:link[%d]\r\n", p_link - &App.link[0]);
        }
...
TAvrcpLink* AllocateNewLinkInst(TBdAddr bd)
{
    ...
    for (int i = 0; i < MAX_AVRCP_APP_INST; i++) //no new avrcp inst, overwrite an old(bd != 0) inst(except partner link)
    {
        TAvrcpLink *p_link = &App.link[i];
        if (p_link->state != avrcpStateConnected && p_link != App.p_partner_link)
        {
            return p_link;
        }
    }
...
void HandleMessageConnected(TBdAddr bd)
{
    ...
    {
        if (p_link->remote_company_id != MY_COMPANY_ID)
        {
            App.p_phone_link = p_link;
            p_link->cmd_credits = 1;
            p_link->state = avrcpStateConnected;
        ...
        {
            App.p_partner_link = p_link;
            printf("old Partner found:link[%d]\r\n", p_link - &App.link[0]);
        }
...
void MyVendorCmdHandler(TBdAddr bd, uint32_t company_id, uint8_t *p_pdu, uint16_t length, uint8_t ctype, uint8_t transact)
{
    ...
    uint8_t response;
    if (company_id == MY_COMPANY_ID && p_link == App.p_partner_link)
    {
        switch (p_pdu[0])
        {
        case VENDOR_CMD_KEY_ACT:
            if (App.p_phone_link != NULL)
            {
                if (App.p_phone_link->state == avrcpStateDisconnected)
                {
                    App.p_phone_link->state = avrcpStateOutgoingConnecting;
                    avrcp_Connect(App.p_phone_link->remote_bd);
                    printf("reconnect[%d]...\r\n", App.p_phone_link - &App.link[0]);
                }
                bool pressed = p_pdu[1] & 0x80 == 0 ? true : false;
                uint8_t key = p_pdu[1] & 0x7f;
                if (pressed)
                {
                    KeyPress(App.p_phone_link, key);
                } else
                {
                    KeyRelease(App.p_phone_link, key);
                }
                response = AVRCP_ACCEPTED;
            } else
            {
                response = AVRCP_REJECTED;
            }
            break;
        default:
    ...
    avrcp_SendVendorResponse(bd, transact, response, company_id, p_pdu, length);
}
...
void AvrcpMessageHandler(TBdAddr bd, AvrcpUsMessageType message_type, void *message)
{
    if (/*message_type >= AVRCP_US_MESSAGE_CONNECT_REQ &&*/message_type <= AVRCP_US_MESSAGE_DISCONNECTED)
    {
        switch (message_type)
        {
        case AVRCP_US_MESSAGE_AUTHORIZATION_IND:
            memcpy(App.authorization_bd, bd, 6);
            printf("authorization ind from[...%x:%x:%x]:press k/l to accept/refuse...\r\n", bd[2], bd[1], bd[0]);
            break;
...
{
    static TAvrcpLink *p_debug_link = &App.link[0];
    printf("link[%d]:%c\r\n", p_debug_link - &App.link[0], rx);

//  
    if (rx == '+')
    {
        VolumeUp(p_debug_link);
    } else if (rx == '-')
    {
        VolumeDown(p_debug_link);
    } else if (rx == 'k')
    {
        GAPBondlegacy_Authorize(App.authorization_bd, blueAPI_CauseAccept);
    } else if (rx == 'l')
    {
        GAPBondlegacy_Authorize(App.authorization_bd, blueAPI_CauseReject);
    } else if (rx == 'u') //update play status
    {
        PendCmdAction(p_debug_link, CmdAction_GetPlayStatus, 0);
        PendCmdAction(p_debug_link, CmdAction_GetElementAttributes, 0);
    } else if (rx == '>') //switch active inst -> to app layer
    {
        int index = (p_debug_link - &App.link[0] + 1) % MAX_AVRCP_APP_INST; //0->1->(Max - 1)->0
        p_debug_link = &App.link[index];
        printf("debug link->%d\r\n", index);
        return;
    } else if (rx == 'i')
    {
        printf("\r\n------info of app:-------\r\n"); //
        //extern xTaskHandle  hLegacyAppTaskHandle;
        //unsigned portBASE_TYPE uxTaskGetStackHighWaterMark( xTaskHandle xTask );
        //printf("StackHighWaterMark of AppTask:%d\r\n", uxTaskGetStackHighWaterMark(hLegacyAppTaskHandle));
        printf("remote bd[%x:%x:%x:%x:%x:%x]\r\n", p_debug_link->remote_bd[5], p_debug_link->remote_bd[4], p_debug_link->remote_bd[3],
               p_debug_link->remote_bd[2], p_debug_link->remote_bd[1], p_debug_link->remote_bd[0]);
        printf("state=%x\r\n", p_debug_link->state);
        printf("company_id=%x\r\n", p_debug_link->remote_company_id);
        /*events*/
        uint16_t tmp = p_debug_link->remote_events_supported;
        int i = 0;
    ...
    {
        TBdAddr bd = { 0x1a, 0x19, 0x48, 0x25, 0x80, 0x0 }; //stollman 1
        avrcp_Connect(bd);
        printf("connect...\r\n");
    } else if (rx == 'w')
    {
        TBdAddr bd = { 0xef, 0x00, 0x48, 0x25, 0x80, 0x0 }; //stollman 2
        avrcp_Connect(bd);
        printf("connect...\r\n");
    } else if (rx == 'e')
    {
        TBdAddr bd = { 0x7b, 0xf4, 0x10, 0x14, 0xe3, 0x24 }; //my phone
        avrcp_Connect(bd);
        printf("connect...\r\n");
    } else if (rx == 'd')
    {
        avrcp_DisConnect(p_debug_link->remote_bd);
        printf("disconnecting link[%d]...\r\n", p_debug_link - &App.link[0]);
    } else
    {
        ...
        if (key != 0)
        {
            if (App.p_phone_link == NULL/*no remote phone*/ && App.p_partner_link != NULL /*partner found*/) //to partner
            {
                if (App.p_partner_link->state == avrcpStateDisconnected)
                {
                    App.p_partner_link->state = avrcpStateOutgoingConnecting;
                    avrcp_Connect(App.p_partner_link->remote_bd);
                    printf("reconnect[%d]...\r\n", App.p_partner_link - &App.link[0]);
                }
                //press, then release when recv rsp
                PendCmdAction(App.p_partner_link, CmdAction_SendVendorCmdKeyAct, (0 << 7) | key);
                PendCmdAction(App.p_partner_link, CmdAction_SendVendorCmdKeyAct, (1 << 7) | key);
            } else //to phone
            {
                {
                    if (p_debug_link->state == avrcpStateIdle)
                    {
                        printf("no remote device\r\n");
                        return;
                    }

                    if (p_debug_link->state == avrcpStateDisconnected)
                    {
                        p_debug_link->state = avrcpStateOutgoingConnecting;
                        avrcp_Connect(p_debug_link->remote_bd);
                        printf("reconnect[%d]...\r\n", p_debug_link - &App.link[0]);
                    }
                }
                ...
                if (rx >= 'a') //lower case
                {
                    ...
                    {
                        pressed = false;
                        KeyRelease(p_debug_link, (uint8_t)key & 0x7f);
                    } else //press and then auto release on cmdCredit gained
                    {
                        KeyPress(p_debug_link, (uint8_t)key & 0x7f);
                        KeyRelease(p_debug_link, (uint8_t)key & 0x7f);
                    }
