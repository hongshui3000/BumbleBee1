...
portBASE_TYPE xReturn = pdFAIL;
	...
	if( xTimerQueue != NULL )
	{
		#if ( INCLUDE_xTimerGetTimerDaemonTaskHandle == 1 )
		{
			...
			xReturn = xTaskCreate( prvTimerTask, ( const char * ) "Tmr Svc", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, &xTimerTaskHandle );
		}
		#else
		{
			/* Create the timer task without storing its handle. */
			xReturn = xTaskCreate( prvTimerTask, ( const char * ) "Tmr Svc", ( unsigned short ) configTIMER_TASK_STACK_DEPTH, NULL, ( ( unsigned portBASE_TYPE ) configTIMER_TASK_PRIORITY ) | portPRIVILEGE_BIT, NULL);
		}
		#endif
	}
...
xTIMER *pxNewTimer;
	...
	{
		pxNewTimer = ( xTIMER * ) pvPortMalloc( RAM_TYPE_DATA_ON, sizeof( xTIMER ) );
		if( pxNewTimer != NULL )
		{
