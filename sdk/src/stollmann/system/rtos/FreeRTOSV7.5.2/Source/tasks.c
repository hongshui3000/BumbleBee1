...
portBASE_TYPE xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, ( char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL );  /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	}
...
tskTCB *pxNewTCB;
	...
	pxNewTCB = ( tskTCB * ) pvPortMalloc( RAM_TYPE_DATA_ON, sizeof( tskTCB ) );
	...
	if( pxNewTCB != NULL )
	{
		...
		pxNewTCB->pxStack = ( portSTACK_TYPE * ) pvPortMallocAligned( RAM_TYPE_DATA_OFF, ( ( ( size_t ) usStackDepth ) * sizeof( portSTACK_TYPE ) ), puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
...
/*-----------------------------------------------------------*/
	...
	char cStatus;
		...
		*pcWriteBuffer = 0x00;
		...
		pxTaskStatusArray = pvPortMalloc( RAM_TYPE_DATA_ON, uxCurrentNumberOfTasks * sizeof( xTaskStatusType ) );
