...
#define IS_UART_IT_ID(ID) (((ID) == UART_IT_ID_LINE_STATUS) || ((ID) == UART_IT_ID_RX_LEVEL_REACH)\
                            || ((ID) == UART_IT_ID_RX_TMEOUT) || ((ID) == UART_IT_ID_TX_EMPTY)\
                            || ((ID) == UART_IT_ID_MODEM_STATUS))
/**
  * @}
  */
...
#define UART_STOP_BITS_NO                           ((uint16_t)(0 << 2))
#define UART_STOP_BITS_1                            ((uint16_t)(1 << 2))

#define IS_UART_STOPBITS(STOP) (((STOP) == UART_STOP_BITS_NO) || ((STOP) == UART_STOP_BITS_1))
/**
  * @}
  */
