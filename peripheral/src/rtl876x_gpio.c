...
void GPIO_INTConfig(uint32_t GPIO_Pin, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        if(GPIO_Pin & 0x3C)
        {
            *((volatile uint32_t *)0x4000600C) |= ((GPIO_Pin & 0x3C) >>1 );
        }
        if(GPIO_Pin & 0xFFFFFFC0)
        {
            *((volatile uint32_t *)0x4000600C) |= BIT(16);
        }
#endif
    }
    else
...
{   
    ...
    GPIO->INTCLR = GPIO_Pin;
        ...
        if(GPIO_Pin & 0x3C)
        {
            *((volatile uint32_t *)0x40006004) = (GPIO_Pin&0x3C) >> 1;
        }
        
        if(GPIO_Pin & 0xFFFFFFC0)
        {
            *((volatile uint32_t *)0x40006004) = BIT(16);
        }
#endif
}
