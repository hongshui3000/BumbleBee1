...
TCB_t *pxNewTCB;
	...
	#if( portSTACK_GROWTH > 0 )
	{
		...
		pxNewTCB = ( TCB_t * ) pvPortMalloc( RAM_TYPE_DATA_ON, sizeof( TCB_t ) );
		...
		if( pxNewTCB != NULL )
		{
			...
			pxNewTCB->pxStack = ( StackType_t * ) pvPortMallocAligned( RAM_TYPE_DATA_ON, ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ), puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
	...
	StackType_t *pxStack;
		...
		/* Allocate space for the stack used by the task being created. */
		pxStack = ( StackType_t * ) pvPortMallocAligned( RAM_TYPE_DATA_OFF, ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ), puxStackBuffer ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
...
/*-----------------------------------------------------------*/
	...
	char cStatus;
		...
		*pcWriteBuffer = 0x00;
		...
		pxTaskStatusArray = pvPortMalloc( RAM_TYPE_DATA_ON, uxCurrentNumberOfTasks * sizeof( TaskStatus_t ) );
